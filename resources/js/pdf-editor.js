...
// Configuration propre à la page hôte (définie dans le <script> Blade juste avant
// @vite, voir resources/views/livewire/partials/pdf-editor-app.blade.php) :
// - initialFileUrl : si défini, charge ce PDF automatiquement au démarrage au lieu
//   de l'état vide, et verrouille la source (impossible d'en ouvrir un autre — voir
//   plus bas) ;
// - saveMode "upload" : "Enregistrer" envoie le PDF au serveur (saveUrl) au lieu de
//   le télécharger dans le navigateur, puis redirige vers afterSaveUrl.
const PAGE_CONFIG = window.PDF_EDITOR_CONFIG || {};

/* ------------------------------------------------------------------------ */
/* Worker pdf.js                                                            */
/* ------------------------------------------------------------------------ */
// L'ancienne version (app statique) vérifiait elle-même l'empreinte SHA-384
// du worker avant de l'utiliser, car il était chargé dynamiquement (`new
// Worker(url)`, API sans attribut `integrity`) depuis un CDN tiers. Ici, le
// worker est bundlé par Vite depuis `node_modules/pdfjs-dist` — sa version
// exacte est pinnée dans package-lock.json et il est servi depuis notre
// propre origine, comme le reste des assets. Le modèle de menace d'un CDN
// tiers compromis ne s'applique donc plus ; l'intégrité vient du lockfile et
// de notre propre pipeline de déploiement, comme pour toute autre dépendance
// bundlée.
...
["dragenter", "dragover"].forEach(evt => el.viewer.addEventListener(evt, (e) => {
  e.preventDefault();
  if (PAGE_CONFIG.initialFileUrl) return;
...
el.viewer.addEventListener("drop", (e) => {
  // Un fichier imposé par la page hôte (voir PAGE_CONFIG plus haut) ne doit pas
  // pouvoir être remplacé par un PDF arbitraire avant l'enregistrement.
  if (PAGE_CONFIG.initialFileUrl) return;
...
async function exportPdf() {
  ...
  try {
    ...
    if (PAGE_CONFIG.saveMode === "upload") {
      await uploadPdf(bytes);
      return; // navigation vers afterSaveUrl en cas de succès ; le finally ci-dessous
               // ne réinitialise le bouton que si uploadPdf a levé une exception avant.
    }
    ...
    console.error(err);
    showError(
      PAGE_CONFIG.saveMode === "upload"
        ? "L'enregistrement a échoué. Vérifiez votre connexion et réessayez."
        : "Une erreur est survenue lors de l'export du PDF."
    );
  } finally {
    el.btnSave.disabled = false;
...
/** Envoie le PDF exporté au serveur (PAGE_CONFIG.saveUrl) au lieu de le télécharger,
 *  puis redirige vers PAGE_CONFIG.afterSaveUrl. Lève une exception sur tout échec
 *  (réseau ou statut HTTP non-2xx), attrapée par l'appelant (exportPdf). */
async function uploadPdf(bytes) {
  const formData = new FormData();
  formData.append("pdf", new Blob([bytes], { type: "application/pdf" }), "document.pdf");

  const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content || "";
  const res = await fetch(PAGE_CONFIG.saveUrl, {
    method: "POST",
  ...
  window.location.href = PAGE_CONFIG.afterSaveUrl || window.location.href;
}

/* ------------------------------------------------------------------------ */
/* Init                                                                      */
/* ------------------------------------------------------------------------ */
/* La modale "À propos" est gérée par flux:modal / Livewire côté Blade, plus
   par ce script (voir resources/views/livewire/pdf-editor.blade.php). */

setTool("select");

if (PAGE_CONFIG.initialFileUrl) {
  // Source imposée par la page hôte (ex. modèle d'attestation) : on masque les
  // moyens d'ouvrir un autre fichier (le drag & drop est déjà neutralisé plus haut).
  el.btnOpen.style.display = "none";
  el.btnOpenEmpty.style.display = "none";

  fetch(PAGE_CONFIG.initialFileUrl)
    .then((res) => {
      if (!res.ok) throw new Error(`TEMPLATE_FETCH_FAILED (${res.status})`);
      return res.blob();
    })
    .then((blob) => loadFile(new File([blob], "document.pdf", { type: "application/pdf" })))
    .catch((err) => {
      console.error(err);
      showError("Impossible de charger le modèle. Rechargez la page.");
    });
}
